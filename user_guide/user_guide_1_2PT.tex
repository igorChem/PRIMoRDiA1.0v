...
\begin{minipage}{\fullwidth}
	\begin{table}[H]
		...
		\begin{tabular}{c|c|c|c}
			...
			\makecell{Densidade \\eletrônica do\\ anion ($\rho^{N+1}(r)$)} &Elec\_Dens\_Anion&$\sum_{i}^{occ} |\phi(r)_i|$ &   \makecell{Escrito se foi utilizado\\ diferenças finitas.} \\\hline
			...
			\makecell{Dureza Local \\(Potencial\\ de Fukui) ($h^{-}(r)$)}  & Fukui\_pot\_left  & $\int   \frac{f^{-}(r')}{|r - r'|}dr'$ &  \makecell{Equação de trabalho\\ que aproxima a densidade \\de carga para $f^{-}$.} \\ \hline
...
\begin{minipage}{\fullwidth}
	\begin{table}[H]
		...
		\begin{tabular}{c|c|c|c}
			...
			\makecell{Susceptibilidade \\a ataque eletrofílico\\($f^-(k)$) } & nucleophilicity & &    \\  \hline	
			\makecell{Susceptibilidade \\a ataque nucleofílico\\($f^+(k)$)}& electrophilicity  & &    \\ \hline	
			\makecell{Susceptibilidade \\a ataque radical \\($f^0(k))$}& radicality & &    \\ \hline
