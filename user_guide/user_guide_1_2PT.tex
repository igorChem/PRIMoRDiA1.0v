...
\begin{minipage}{\fullwidth}
	\begin{table}[H]
		...
		\begin{tabular}{c|c|c|c|c}
			\toprule
			Descritor &\makecell{Nome do\\ arquivo\\ cube}&\makecell{Definição\\Matemática}&Descrição&Observação \\
			\midrule
			\makecell{Densidade \\eletrônica ($\rho(r)$)} &Elec\_Dens &$\sum_{i}^{occ} |\phi(r)^2_i|$ & & \\\hline
			\makecell{Densidade \\eletrônica\\ do cátion} &Elec\_Dens\_Cation &$\sum_{i}^{occ} |\phi(r)^2_i|$ & & \\\hline
			\makecell{Densidade \\eletrônica\\ do anion} &Elec\_Dens\_Anion&$\sum_{i}^{occ} |\phi(r)_i|$ & & \\\hline
			\makecell{Susceptibilidade\\eletrofílica} &left\_Fukui&$\lim_{\partial N \to 0^-}\left(\frac{\partial  \rho(r)}{\partial N} \right)_\nu$ &  &  \\  \hline	
			\makecell{Susceptibilidade\\nucleofílica}&right\_Fukui&$\lim_{\partial N \to 0^+}\left(\frac{\partial  \rho(r)}{\partial N} \right)_\nu$  &   & \\ \hline	
			\makecell{Susceptibilidade\\radical}&zero\_Fukui&$\lim_{\partial N \to 0}\left(\frac{\partial  \rho(r)}{\partial N} \right)_\nu$  &   & \\ \hline
			\makecell{Descritor Dual}&net\_Fukui &$\left(\frac{\partial ^2 \rho(r)}{\partial N} \right)_\nu$ & & \\ \hline
			\makecell{Dureza Local}&l\_hardness\_Vee&$\frac{1}{2N} \int \frac{\rho(r')}{|r-r'|} dr'$ & & \\ \hline			
			\makecell{Dureza Local\\(Potencial Químico)}  & l\_hardness\_lcp &  & & \\ \hline
			\makecell{Dureza Local\\(Potencial de Fukui)}  &Fukui\_pot\_left  &  & & \\ \hline
			\makecell{Moleza\\Local Dual}  & softness\_dual &  & &  \\ \hline
			\makecell{Hiper Moleza\\Local}  & hyper\_softness &  & &  \\ \hline
			\makecell{Multifilicidade\\Local}  & multifilicity &  & &  \\
			\bottomrule
		\end{tabular} 
		\label{tab_local1}	
	\end{table}	
...
\begin{minipage}{\fullwidth}
	\begin{table}[H]
		...
		\begin{tabular}{c|c|c|c|c}
			\toprule
			Descritor &\makecell{Nome no\\ arquivo}&\makecell{Definição\\Matemática}&Descrição&Observação \\
			\midrule
			\makecell{Densidade \\eletrônica} & & & & \\\hline
			\makecell{Carga parcial} & & & & \\\hline
			\makecell{Susceptibilidade\\eletrofílica} &  & &  &  \\  \hline	
			\makecell{Susceptibilidade\\nucleofílica}&  & &   & \\ \hline	
			\makecell{Susceptibilidade\\radical}&  & &   & \\ \hline
			\makecell{Descritor dual}  & & & & \\ \hline			
			\makecell{Dureza local\\(potencial\\elétron-elétron)}  &  &  & & \\ \hline
			\makecell{Dureza local\\(potencial químico)}  &  &  & & \\ \hline
			\makecell{Dureza local\\(potencial\\fukui)}  &  &  & & \\ \hline
			\makecell{Moleza\\local dual}  &  &  & &  \\ \hline			
			\makecell{Hiper moleza\\local}  &  &  & &  \\ \hline
			\makecell{Multifilicidade\\local}  &  &  & &  \\\hline
			\makecell{Eletrofilicidade\\total local}  &  &  & &  \\\hline
			\makecell{Localização\\de banda\\de fronteira}  &  &  & &  \\
			\bottomrule
		\end{tabular} 
		\label{tab_local2}	
	\end{table}	
...
\begin{minipage}{\fullwidth}
	\begin{table}[H]
		...
		\begin{tabular}{c|c|c|c|c}
			...
			\makecell{Potencial Fukui\\ nucleofílico} & & & & \\\hline
			\makecell{Potencial Fukui\\ radical} & & & & \\\hline
			\makecell{Moleza\\ dual média} & & & & \\\hline
			\makecell{Potencial\\eletrostático molecular } &$\sum_A\frac{Z_A}{|r-R_A|}  - \int  \frac{\rho(r')}{|r-r'|} dr'$ & & & \\\hline
			\makecell{Dureza\\local (TFD)} & & & & \\
			\bottomrule
		\end{tabular} 
		\label{tab_local3}	
	\end{table}	
...
\begin{minipage}{\fullwidth}
	\begin{table}[H]
		...
		\begin{tabular}{c|c|c|c|c}
			...
			\makecell{Potencial\\Fukui nucleofílico}  &  &  & & \\ \hline
			\makecell{Potencial\\Fukui radical}  &  &  & & \\ \hline
			\makecell{Potencial\\molecular eletrostático}  &  &  & & \\ \hline
			\makecell{Moleza\\local média}  &  &  & &  \\ \hline			
			\makecell{Dureza\\local (TFD)} & & & & \\
			\bottomrule
		\end{tabular} 
		\label{tab_local4}	
