...
void Imolecule::print(){
	cout << "Molecule's name: " << name << endl;
	this->print_coordinates();
	cout << "The coeff are normalized :"	<< normalized	<< endl;
	cout << "LUMO energy  : "				<< lumo_energy 	<< endl;
	cout << "HOMO energy  : "				<< homo_energy	<< endl;
	cout << "Total energy : "				<< energy_tot	<< endl;
	cout << "Printing the first ten values of the vector in this object" << endl;
...
void Imolecule::update(){
	...
	if ( num_of_electrons == 0 ){
		...
		m_log->write_warning("Starting to count based on atomic numbers!!");
		for(unsigned int i=0;i<atoms.size();i++){
...
bool Imolecule::check(){
	...
	if ( num_of_atoms == 0 || atoms.size() == 0 ){
		m_log->write_warning("Information of atoms not stored"); 
		all_ok = false;
	}
	if ( num_of_electrons == 0 ){
		m_log->write_warning("Zero electrons counted for the molecule!"); 
		all_ok = false;
	}
	if ( num_of_ao == 0 ){
		m_log->write_warning("Zero atomic orbitals counted for the molecule!"); 
		all_ok = false;
	}
	if ( atoms[0].orbitals.size() == 0 ){
		m_log->write_warning("Zero atomic orbitals parsed from file for this molecule!"); 
		all_ok = false;
	}
	if ( coeff_MO.size() == 0 ){
		m_log->write_warning("Molecular orbitals coeffcients not stored"); 
		all_ok = false;
	}
	if ( orb_energies.size() == 0 || MOnmb == 0) {
		m_log->write_warning("Molecular orbitals energies not stored");  
		all_ok = false;
	}
	...
	if ( occupied.size() == 0 ){
		m_log->write_warning("Molecular Orbital occupation information was not stored.");
		all_ok = false;
	...
	if ( homo_energy == 0.0000 || homoN == 0 ){
		m_log->write_warning("HOMO energy may bee wrong!");
		all_ok = false;
	}
	if ( lumo_energy == 0.0000 || lumoN == 0 ){
		m_log->write_warning("LUMO energy may bee wrong!");
		all_ok = false;
	}
	...
	if ( mol_charge_test == 0.000 ){
		m_log->write_warning("Charges stored in atoms objects are not consistent!");
		all_ok = false;
	}
	
	if ( betad ){
		if ( coeff_MO_beta.size() == 0 || MOnmb_beta == 0 ){
			m_log->write_warning("Your QM output is said to contain beta set molecular orbitals, but they were not store.");
			all_ok = false;
		}
		if ( orb_energies_beta.size() == 0 ){
			m_log->write_warning("Your QM output is said to contain beta set molecular orbitals, but their energies  were not store.");
			all_ok = false;
		}
	}	
	if ( !all_ok) {
...
/***************************************************************************************/
void Imolecule::light_copy(Imolecule& mol){
	mol.name 		= name;
	mol.atoms		= atoms;
	mol.mol_charge	= mol_charge;
}
/***************************************************************************************/
double Imolecule::calc_dist(int a1, int a2){
	double xtemp = atoms[a1].xcoord - atoms[a2].xcoord;
	double ytemp = atoms[a1].ycoord - atoms[a2].ycoord;
	double ztemp = atoms[a1].zcoord - atoms[a2].zcoord;
	xtemp *= xtemp;
	ytemp *= ytemp;
	ztemp *= ztemp;
	double result = sqrt( xtemp + ytemp + ztemp);
	return result;
}
/***************************************************************************************/
void Imolecule::write_pdb(){
	std::ofstream pmol_file;
	string fname = name+".pdb"; 
	pmol_file.open( fname.c_str() );
	pmol_file << std::fixed;
	pmol_file.precision(3);
		
	pmol_file << "TITLE PDB written from molecular info by PRIMoRDiA Software\n";	
		
	
	for( unsigned i=0; i<atoms.size(); i++ ){
		pmol_file	<< std::setw(6) << std::left  << "ATOM" 
					<< " "
					<< std::setw(4) << std::right  << (i+1) 
					<< " "
					<< std::setw(4) << atoms[i].element 
					<< " "
					<< std::left << std::setw(4) << "MOL" 
					<< " "
					<< std::right << std::setw(4) << 1
					<< std::setw(5) << " "
					<< std::setw(7) << atoms[i].xcoord 
					<< " "
					<< std::setw(7) << atoms[i].ycoord
					<< " "
					<< std::setw(7) << atoms[i].zcoord 
					<< " "
					<< std::setw(5)  << "1.00"
					<< " "
					<< std::setw(5) << "0.00"
