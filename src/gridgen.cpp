...
gridgen::gridgen(int grd					, 
	...
	for (unsigned int j=0;j<molecule.num_of_atoms;j++){
		for (unsigned int k=0;k<molecule.atoms[j].norb;k++){
	...
	if ( orbs.size() <= molecule.num_of_atoms ){
		m_log->input_message("Your atomic basis may not be loaded properly. Verify your QM data and/or use -log option to run PRIMoRDiA.");
	}
...
void gridgen::write_grid(){
	...
	string cb_name2;
	if ( orbital ) {
		std::string nmo = int_to_string(Norb);
