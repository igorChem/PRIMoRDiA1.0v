...
gaussian_files::gaussian_files(const char* file_name):
	...
	if ( IF_file(file_name) ) {
		...
		m_log->input_message("Error opening GAUSSIAN file! Verify its presence in the current directory!");
...
void gaussian_files::parse_fchk(){
	...
	unique_ptr<Ibuffer> b_file( new Ibuffer(name_f,true) );
	...
	for(i=0;i<b_file->lines.size();i++){
		if ( i>atomic_n_i && i<atomic_n_f){
			...
			for(j=0;j<b_file->lines[i].line_len;j++){
				molecule->atoms[k++].charge = b_file->lines[i].pop_double(0);
			}
		}
	}
...
void gaussian_files::get_overlap_m(){
	...
	if ( IF_file(log_name.c_str() ) ){
		if ( !check_file_ext(".log",log_name.c_str() ) ) {
			cout << "Warning! The file has wrong etension name!" << endl;
			m_log->input_message("Warning! The file has wrong etension name!");
			m_log->input_message("Error in open file to parse overlap matrix!");
		}
		molecule->m_overlap.resize( ( molecule->MOnmb*(molecule->MOnmb+1) )/2 );
		
		unsigned int over_in, over_fin;
		buffer.reset(new Ibuffer(log_name.c_str(),true));
		...
		m_log->input_message("Log file not opening! Error in parse overlap matrix for gaussian file.");
	}
}
/******************************************************************************/
gaussian_files::~gaussian_files(){}
