...
void local_rd_cnd::calculate_Fukui(){
	...
	if ( !finite_diff ) {
		...
		if ( qm_calc->data_ok ){
			...
			m_log->input_message("Skipping Local RD calculations!");
		}
...
void local_rd_cnd::write_LRD(){
	...
	lrd_file << std::fixed;
	lrd_file << name << " " << "\n" <<  std::left;
	lrd_file << "n atom charge EAS NAS RAS Netphilicity Softness Hardness_A Hardness_B Hardness_C Hardness_D Multiphilic Electrohilicity Fukushima Electron_density Softness_dual\n";
